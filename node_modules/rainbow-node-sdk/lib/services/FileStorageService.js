...
class FileStorage {
    ...
    start(__xmpp, __rest, __fileServerService, __conversations) {
        let that = this;
    ...
    getFilesReceivedInConversation(conversation) {
        ...
        return new Promise(function (resolve, reject) {
            if (!conversation) {
                ...
                that.retrieveFilesReceivedFromPeer(/* contactService.userContact.dbId */ that._rest.userId, conversation.contact.id).then(function (files) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (that.fileDescriptors.some((fd) => {
                ...
                for (let viewer of fileDescriptor.viewers) {
                    if (viewer.type === "user") {
                        promiseArray.push(that.contactService.getContactById(viewer.viewerId, true).then((contact) => {
        ...
        if (data) {
            ...
            if (data.viewers) {
                for (let viewerData of data.viewers) {
                    viewers.push(FileViewer_1.FileViewerElementFactory(viewerData.viewerId, viewerData.type, viewerData.contact, that.contactService));
                }
            }
            let url = data.url;
            if (!url) {
