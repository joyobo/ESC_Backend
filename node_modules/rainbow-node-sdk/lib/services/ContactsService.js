...
class Contacts {
    ...
    start(_xmpp, _rest, _invitationsService, _presenceService) {
        let that = this;
    ...
    init() {
        return new Promise((resolve, reject) => {
            let that = this;
            let userInfo = that.getContactById(that.rest.account.id, true).then((contact) => {
            ...
            return new Promise((resolve, reject) => {
                if (!loginEmail) {
                    ...
                    if (contactFound) {
                        ...
                        that.rest.getContactInformationByLoginEmail(loginEmail).then((contactsFromServeur) => __awaiter(this, void 0, void 0, function* () {
    ...
    isUserContact(contact) {
        ...
        if (!that.rest.account) {
            return (contact.jid === that.xmpp.jid);
        }
                ...
                if (contact.subscription === "both") {
                    if (!that.contacts.find(item => {
                        ...
                        that
                            ...
                            that.rosterPresenceQueue.filter(presenceItem => presenceItem.presence.jid === contact.jid).forEach(item => that._onRosterPresenceChanged(item.presence));
