...
module.exports.toggle_engaged = function toggle_engaged(changeTo, agentId){
    ...
    let selectQuery = mysql.format('SELECT Engaged FROM agents_iphone WHERE AgentId = ?',[agentId]);
    pool.query(selectQuery,(err,result) => {
        if(err) console.log(err);
        else{
